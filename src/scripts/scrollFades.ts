/**
 * Drives top/bottom edge fades over a scrolling element by toggling `at-start` / `at-end` classes on a target element. Page CSS hides the fade for whichever edge the reader has already reached:
 *
 *   .target.at-start .fade-top    { opacity: 0 }
 *   .target.at-end   .fade-bottom { opacity: 0 }
 *
 * Both classes are set when the content doesn't overflow, so neither fade shows when there's nothing to hint at.
 *
 * Returns a manual `update` for callers that reshape the content themselves (filtering rows, animating open) without firing a scroll or resize event.
 */
export interface ScrollFadesOptions {
  ...
  target?: HTMLElement;
  /** element whose resize should recompute the state — e.g. a `<pre>` that grows taller when a web font swaps in, which changes overflow without any scroll or resize event (default: the scroller) */
